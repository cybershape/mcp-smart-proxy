...
- Keep popup window placement and activation pragmatic in the Swift helper: remember the last closed window position, restore it only when that placement is still visibly on-screen, fall back to the default placement otherwise, keep the window at normal level instead of staying frontmost so IME/system overlays remain visible, and keep the default auto-select timeout at 120 seconds.
